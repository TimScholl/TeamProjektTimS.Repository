...
class AvoidMovement(Movement):
    # TODO testing

    def vision(self, data, robot):
        robot.destination = AvoidMovement.prime_object(data)
        return robot.a, robot.a_alpha
    ...
    def position(self, data, robot):
        ...
        print(f"pos: {obj_coordinates}    type: {obj_type}    distance: {obj_distance}")
        print("------------------------------------------------------------------------")

        # calculate object angle
        obj_angle = AvoidMovement.calculate_angle_between_vectors(obj_coordinates, x, y, v, alpha)

        # calculate vectors
        velocity_vector = AvoidMovement.calculate_vector(v, alpha)
        object_vector = AvoidMovement.calculate_vector(obj_distance, obj_angle)

        # set Threshold
        threshold = AvoidMovement.calculate_threshold(obj_type, v, alpha, v_alpha, v_max, robot)

        # getting information about angle and direction
        turn_direction = AvoidMovement.calculate_direction(object_vector, velocity_vector)
        delta_alpha = AvoidMovement.set_delta_alpha(obj_type,obj_distance,threshold,obj_angle,turn_direction,v_alpha)
        print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        ...
        a = AvoidMovement.set_acceleration(v, v_max)
        a_alpha = AvoidMovement.set_angle_acceleration(turn_direction, delta_alpha, v_alpha, robot)

        # print info
        AvoidMovement.print_info(False, velocity_vector, object_vector, threshold, turn_direction,
                                 delta_alpha, a, a_alpha)
